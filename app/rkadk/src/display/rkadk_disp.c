...
static int RKADK_DISP_CreateVo(RKADK_U32 VoLayer, RKADK_U32 VoDev,
  ...
  stChnAttr.u32Priority = pstDispCfg->vo_chn;
...
#ifdef RV1126B
  stGrpAttr.enVProcDev = VIDEO_PROC_DEV_RGA;
#endif
...
RKADK_S32 RKADK_DISP_SetAttr(RKADK_U32 u32CamId, RKADK_DISP_ATTR_S *pstAttr) {
  ...
  if (ret) {
    RKADK_LOGE("flush vo [layer: %d, chn: %d] failed!, ret: %x", pstDispCfg->vo_layer, ret);
    return ret;
  }
#endif
