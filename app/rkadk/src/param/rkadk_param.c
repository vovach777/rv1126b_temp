...
static void RKADK_PARAM_Dump() {
  ...
  for (i = 0; i < (int)pstCfg->stCommCfg.sensor_count; i++) {
    ...
    for (j = 0; j < (int)pstCfg->stMediaCfg[i].stRecCfg.file_num; j++) {
      ...
      printf("\t\tsensor[%d] stRecCfg[%d] flt_str_i: %d\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.flt_str_i);
      printf("\t\tsensor[%d] stRecCfg[%d] flt_str_p: %d\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.flt_str_p);
      printf("\t\tsensor[%d] stRecCfg[%d] aq_step_i: %s\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.aq_step_i);
      printf("\t\tsensor[%d] stRecCfg[%d] aq_step_p: %s\n", i, j,
             pstCfg->stMediaCfg[i].stRecCfg.attribute[j].venc_param.aq_step_p);
    }
    ...
    printf("\t\tsensor[%d] stStreamCfg flt_str_i: %d\n", i,
            pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.flt_str_i);
    printf("\t\tsensor[%d] stStreamCfg flt_str_p: %d\n", i,
            pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.flt_str_p);
    printf("\t\tsensor[%d] stStreamCfg aq_step_i: %s\n", i,
            pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.aq_step_i);
    printf("\t\tsensor[%d] stStreamCfg aq_step_p: %s\n", i,
            pstCfg->stMediaCfg[i].stStreamCfg.attribute.venc_param.aq_step_p);
    ...
    printf("\t\tsensor[%d] stLiveCfg flt_str_i: %d\n", i,
            pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.flt_str_i);
    printf("\t\tsensor[%d] stLiveCfg flt_str_p: %d\n", i,
            pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.flt_str_p);
    printf("\t\tsensor[%d] stLiveCfg aq_step_i: %s\n", i,
            pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.aq_step_i);
    printf("\t\tsensor[%d] stLiveCfg aq_step_p: %s\n", i,
            pstCfg->stMediaCfg[i].stLiveCfg.attribute.venc_param.aq_step_p);
...
static RKADK_S32 RKADK_PARAM_SetRcParam(RKADK_PARAM_VENC_ATTR_S stVencAttr) {
  ...
  // set rc param2
  if (strcmp(stVencAttr.venc_param.aq_step_i, "NONE") || strcmp(stVencAttr.venc_param.aq_step_p, "NONE")) {
    memset(&stRcParam2, 0, sizeof(VENC_RC_PARAM2_S));
    ret = RK_MPI_VENC_GetRcParam2(stVencAttr.venc_chn, &stRcParam2);
    if (ret) {
      RKADK_LOGE("venc_chn[%d] GetRcParam2 failed[%d]", stVencAttr.venc_chn, ret);
      return -1;
    }

    if (strcmp(stVencAttr.venc_param.aq_step_i, "NONE"))
      RKADK_PARAM_Strtok(stVencAttr.venc_param.aq_step_i, stRcParam2.s32AqStepI, 17, ",");

    if (strcmp(stVencAttr.venc_param.aq_step_p, "NONE"))
      RKADK_PARAM_Strtok(stVencAttr.venc_param.aq_step_p, stRcParam2.s32AqStepP, 17, ",");

    ret = RK_MPI_VENC_SetRcParam2(stVencAttr.venc_chn, &stRcParam2);
    if (ret) {
      RKADK_LOGE("venc_chn[%d] SetRcParam2 failed[%d]", stVencAttr.venc_chn, ret);
      return -1;
    }
  }
...
static RKADK_S32 RKADK_PARAM_SetVencFilter(RKADK_PARAM_VENC_ATTR_S stVencAttr) {
  int ret = 0;
  VENC_FILTER_S stFilter;

  if (stVencAttr.venc_param.flt_str_i != -1 || stVencAttr.venc_param.flt_str_p != -1) {
    memset(&stFilter, 0, sizeof(VENC_FILTER_S));
    ret = RK_MPI_VENC_GetFilter(stVencAttr.venc_chn, &stFilter);
    if (ret) {
      RKADK_LOGE("venc_chn[%d] GetFilter failed[%d]", stVencAttr.venc_chn, ret);
      return -1;
    }

    if (stVencAttr.venc_param.flt_str_i >= 0 && stVencAttr.venc_param.flt_str_i <= 3)
      stFilter.u32StrengthI = stVencAttr.venc_param.flt_str_i;

    if (stVencAttr.venc_param.flt_str_p >= 0 && stVencAttr.venc_param.flt_str_p <= 3)
      stFilter.u32StrengthP = stVencAttr.venc_param.flt_str_p;

    ret = RK_MPI_VENC_SetFilter(stVencAttr.venc_chn, &stFilter);
    if (ret) {
      RKADK_LOGE("venc_chn[%d] SetFilter failed[%d]", stVencAttr.venc_chn, ret);
      return -1;
    }
  }
...
RKADK_S32 RKADK_PARAM_SetVAdvancedParam(RKADK_PARAM_VENC_ATTR_S stVencAttr) {
  ...
  ret |= RKADK_PARAM_SetVencFilter(stVencAttr);
