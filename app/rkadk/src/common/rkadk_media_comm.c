...
RKADK_S32 RKADK_MPI_VI_QueryCameraStatus(RKADK_U32 u32CamId) {
  ...
  ret = RK_MPI_VI_QueryDevStatus(pstSensorCfg->dev_id, &stDevStatus);
  if (ret == RK_SUCCESS) {
    if (!stDevStatus.bProbeOk) {
      RKADK_LOGE("Camera[%d] u32DevId[%d] sensor probe failed[%x]", u32CamId, pstSensorCfg->dev_id, ret);
      return -1;
    }
  }
...
RKADK_S32 RKADK_MPI_SYS_Bind(MPP_CHN_S *pstSrcChn,
  ...
  int ret = -1, count;
  RKADK_S32 i;
  RKADK_U32 u32DevId = 0, u32SrcDevId = 0;
  ...
  if (0 == pstInfo[i].s32BindCnt) {
    ...
    u32SrcDevId = pstSrcChn->s32DevId;
  ...
  RKADK_LOGD("src[%d(%d), %d, %d], dest[%d, %d, %d], BindCnt[%d]",
              u32SrcDevId, u32DevId, pstSrcChn->enModId, pstSrcChn->s32ChnId,
