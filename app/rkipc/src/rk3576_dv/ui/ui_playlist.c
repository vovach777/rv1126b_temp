...
static void list_btn_check_next() {
	...
	int child_cnt = lv_obj_get_child_cnt(playlist_ctx.list_obj);
	if (child_cnt <= 0)
		return;
	...
	next_btn_id = (cur_btn_id + 1) % child_cnt;
...
static void btn_click_event_cb(lv_event_t *e) {
	...
	uint32_t track_id = (uint64_t)lv_obj_get_user_data(obj);
	...
	if (code == LV_EVENT_CLICKED) {
		...
		if (track_id < playlist_ctx.list.count) {
			...
			if (playlist_ctx.mode != RK_PHOTO_MODE)
				...
				ui_page_push_page("photo_player", (void *)((uint64_t)track_id));
...
static lv_obj_t *add_list_btn(lv_obj_t *parent, uint32_t track_id) {
	...
	lv_style_set_text_font(&symbol_style, &lv_font_montserrat_20);
	...
	lv_obj_set_user_data(btn, (void *)((uint64_t)track_id));
...
static void scroll_event_cb(lv_event_t *e) {
	...
	if (step_size > 0) {
		...
		track_id = (uint64_t)lv_obj_get_user_data(child);
...
static void playlist_create_ctrl(lv_obj_t *page_obj) {
	...
	obj = page_obj;
	// HACK: there has a style bug, workaround here.
	if (switch_from_player) {
		lv_disp_set_bg_opa(NULL, LV_OPA_COVER);
		lv_disp_set_bg_color(NULL, lv_color_hex(0x343247));
		switch_from_player = false;
	} else {
	...
	lv_obj_add_event_cb(playlist_ctx.menu_obj, menu_event_cb, LV_EVENT_CLICKED, 0);
	...
	lv_style_set_text_font(&style_lable, ttf_info_10.font);
...
static void playlist_add_indev(void) {
	evdev_force_translate_to_click(true);
	lv_group_t *group = lv_port_indev_group_create();
	if (NULL == group)
		return;

	lv_group_add_obj(group, playlist_ctx.menu_obj);

	playlist_ctx.group = group;
}

static void playlist_delete_indev(void) {
	evdev_force_translate_to_click(false);
