...
int rk_pwm_set_polarity(uint32_t pwm, enum pwm_polarity polarity) {
	...
	if (polarity != PWM_POLARITY_INVERSED && polarity != PWM_POLARITY_NORMAL)
		return -EINVAL;
	...
	return write_sysfs_string_and_verify("polarity", path,
	                                     (polarity == PWM_POLARITY_NORMAL) ? "normal" : "inversed");
}
...
int rk_pwm_get_polarity(uint32_t pwm) {
	...
	return (memcmp(string, "inversed", sizeof("inversed"))) ? PWM_POLARITY_NORMAL
...
int rk_pwm_init(uint32_t pwm, uint32_t period, uint32_t duty, enum pwm_polarity polarity) {
	int ret;
	char path[64];
