...
typedef struct rkVPSS_GRP_ATTR_S {
    ...
    RK_U32                     u32MaxQueue;       /* RW; Max input queue length */
    RK_BOOL                    bMirror;           /* RW; Mirror enable. only for hardware vpss */
...
typedef struct rkVPSS_CHN_ATTR_S {
    ...
    RK_BOOL             bMirror;            /* RW; Mirror enable, hardware vpss is invalid */
    ...
    SIZE_S              stMaxSize;          /* RW; chn output max resolution */
    POINT_S             stPoint;            /* RW; output point*/
} VPSS_CHN_ATTR_S;
...
typedef struct rkVPSS_CHN_BUF_WRAP_S {
    RK_BOOL bEnable;
