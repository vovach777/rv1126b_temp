...
typedef enum {
    ROCKIVA_NONVEHICLE_ALERT_NONE,                    /* 不进行区域告警 */
    ROCKIVA_NONVEHICLE_ALERT_IN_NOSTOP_AREA,          /* 停放禁停区域内 */
    ROCKIVA_NONVEHICLE_ALERT_OUT_STOP_AREA,           /* 停放指定区域外 */
} RockIvaNonvehicleAlertType;
...
typedef struct
{
    ...
    uint8_t detScore;       /* 检测分数阈值[1,100] */
    uint32_t minSize;       /* 最小检测非机动车属性尺寸，小于该值过滤 */
    uint32_t intervalTime;  /* 运行时间间隔（单位毫秒） */
    RockIvaNonvehicleAlertType alertType;   /* 告警类型 */
    uint32_t triggerType;   /* 触发类型, 例如： ROCKIVA_OBJECT_TYPE_BITMASK(ROCKIVA_OBJECT_TYPE_MOTORCYCLE) */
    RockIvaAreas roiAreas;  /* 区域 */
    uint32_t alertTime;     /* 告警时间设置（单位毫秒） */
    uint8_t sense;          /* 区域灵敏度,1~100 */
...
typedef struct
{
    ...
    uint8_t alert;                   /* 1: 非机动车告警 */
    RockIvaObjectType alertType;     /* 告警目标类型 */
    uint8_t firstTrigger;            /* 1: 第一次触发非机动告警 */
