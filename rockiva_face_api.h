...
typedef enum {
    ...
    ROCKIVA_FACE_OPT_CYCLE                      /* 识别模式 目标定期抓拍上报直到设置不抓拍 */
} RockIvaFaceOptType;
...
typedef enum {
    ...
    ROCKIVA_FACE_CAP_TYPE_CYCLE,                      /* 人脸周期上报 */
...
typedef struct {
    ...
    uint8_t relatedHeadEnable;                  /* 是否关联人头 1：关联并基于人头跟踪ID过滤重复抓拍 2：objId使用人头跟踪ID替代 */
...
typedef struct {
    ...
    uint32_t optBestOverTime;                     /* 人脸抓拍超时时间设置ms，若人脸在此时段内未消失则上报此时段内的质量最优人脸；若为0，则在消失后才上报质量最优人脸 */
    uint32_t optCycleTime;                        /* 人脸周期抓拍间隔时间ms */
...
/* 单个目标人脸检测基本信息 */
typedef struct {
    uint32_t objId;                                       /* 目标跟踪ID */
    uint32_t faceId;                                      /* 人脸跟踪ID */
